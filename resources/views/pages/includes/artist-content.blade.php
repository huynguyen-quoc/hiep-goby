<div class="full-width bg-white col_padding_bottom">

    <div class="content">
        <div class="loader-container active" >
            <div class="content">
                <div class="gobyArtIcon spinner">S</div>
                <div class="icon-label">Loading</div>
            </div>
        </div>
        <div class="grid-masonry grid-model-list ">
            <div class="grid-column-size"></div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/1.jpg')}}">
                            <img src="{{asset('assets/images/details/1.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/2.jpg')}}" >
                            <img src="{{asset('assets/images/details/2.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/3.jpg')}}">
                            <img src="{{asset('assets/images/details/3.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/4.jpg')}}">
                            <img src="{{asset('assets/images/details/4.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/5.jpg')}}">
                            <img src="{{asset('assets/images/details/5.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/6.jpg')}}">
                            <img src="{{asset('assets/images/details/6.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/7.jpg')}}">
                            <img src="{{asset('assets/images/details/7.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/8.jpg')}}">
                            <img src="{{asset('assets/images/details/8.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/9.jpg')}}">
                            <img src="{{asset('assets/images/details/9.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/10.jpg')}}">
                            <img src="{{asset('assets/images/details/10.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/11.jpg')}}">
                            <img src="{{asset('assets/images/details/11.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/12.jpg')}}">
                            <img src="{{asset('assets/images/details/12.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/13.jpg')}}">
                            <img src="{{asset('assets/images/details/13.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/14.jpg')}}">
                            <img src="{{asset('assets/images/details/14.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/15.jpg')}}">
                            <img src="{{asset('assets/images/details/15.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/16.jpg')}}">
                            <img src="{{asset('assets/images/details/16.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/17.jpg')}}">
                            <img src="{{asset('assets/images/details/17.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>
            <div class="masonry-brick">
                <article class="image grid-item">
                    <div class="model-img-wrapper">
                        <a class="fancybox" href="{{asset('assets/images/details/18.jpg')}}">
                            <img src="{{asset('assets/images/details/18.jpg')}}"  >
                        </a>
                    </div>
                </article>
            </div>


        </div>
    </div>
    <div class="text-center">
        <a class="button-more" href="/models">
            <span class="gobyArtIcon medium">F</span><br>Load More
        </a>
    </div>
</div>